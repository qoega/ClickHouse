...
class FileAndPos:
    ...
    def from_start(self, path):
        if not self.file:
            return self
        return FileAndPos(os.path.relpath(self.file, start=path), self.pos)
...
class FileBlockBase:
    def __init__(self, test_file, start, end):
    ...
    def get_file_and_pos(self):
        return FileAndPos(self._test_file.get_test_name(), self._start + 1)
    ...
    @staticmethod
    def parse_block(test_file, start, end):
        file_pos = FileAndPos(test_file.get_test_name(), start + 1)
        ...
        while line < end:
            ...
            if _is_comment_line(tokens):
                ...
                query = test_file.get_tokens(line)
                request, last_line = FileBlockBase.__parse_request(test_file, line+1, end)
                ...
                result, last_line = FileBlockBase.__parse_result(test_file, line+1, end)
                assert last_line == end
                line = last_line
            line += 1

        if block_type == BlockType.comments:
            return FileBlockComments(test_file, start, end)

        if block_type == BlockType.control:
            return FileBlockControl(test_file, start, end, conditions, controls)

        if block_type == BlockType.statement:
            return FileBlockStatement(test_file, start, end, conditions, statement, request)

        if block_type == BlockType.query:
            block = FileBlockQuery(test_file, start, end, conditions, query, request, result_line)
    ...
    def dump_to(self, output):
        ...
        for line in range(self._start, self._end):
            output.write(self._test_file.get_line(line))
        output.write("\n")


class FileBlockComments(FileBlockBase):
    def __init__(self, test_file, start, end):
        super().__init__(test_file, start, end)
...
class FileBlockControl(FileBlockBase):
    def __init__(self, test_file, start, end, conditions, control):
        super().__init__(test_file, start, end)
...
class FileBlockStatement(FileBlockBase):
    def __init__(self, test_file, start, end, conditions, statement, request):
        super().__init__(test_file, start, end)
        self.conditions = conditions
...
class FileBlockQuery(FileBlockBase):
    def __init__(self, test_file, start, end, conditions, query, request, result_line):
        super().__init__(test_file, start, end)
        self.conditions = conditions
        self.query = query
        self.request = request
    ...
    def get_types(self):
        if self.query[1] == "error":
            raise TestFileFormatException("the query is expected to fail, there are no types")
        return self.query[1]
...
class TestFile:
    ...
    def __load_file(self):
        ...
        logger.debug("Test file %s loaded rows %s, empty rows %s",
                     self._test_name, len(self._lines), len(self._empty_lines))
...
class QueryResult:
    def __init__(self, rows=None, values_count=None, data_hash=None, exception=None, hash_threshold=0):
        self.rows = rows
    ...
    def __str__(self):
        params = ", ".join((str(x) for x in [
            "rows: {}".format(self.rows) if self.rows else "",
            "values_count: {}".format(self.values_count) if self.values_count else "",
            "data_hash: {}".format(self.data_hash) if self.data_hash else "",
            "exception: {}".format(self.exception) if self.exception else "",
            "hash_threshold: {}".format(self.hash_threshold) if self.hash_threshold else "",
        ] if x))
    ...
    def __iter__(self):
        ...
        if self.data_hash is not None:
            return iter([["{} values hashing to {}".format(self.values_count, self.data_hash)]])
    ...
    def parse_it(rows, hash_threshold):
        ...
        return QueryResult(rows=rows, values_count=values_count, hash_threshold=hash_threshold)
    ...
    def make_it(rows, types, sort_mode, hash_threshold):
        ...
        return QueryResult(rows=as_sorted, values_count=values_count, hash_threshold=hash_threshold)
    ...
    def assert_eq(canonic, actual):
        ...
        if None not in (canonic.exception, actual.exception):
        ...
        if canonic.data_hash is not None:
            ...
            if canonic.values_count != actual.values_count:
                raise DataResultDiffer("canonic and actual results have different value count",
                                       details="canonic values count {}, actual {}".format(
                                           canonic.values_count, actual.values_count)
                                       )
            if canonic.data_hash != actual.data_hash:
                raise DataResultDiffer("canonic and actual results have different hashes")
            return
        ...
        if canonic.rows is not None and actual.rows is not None:
            ...
            if canonic.rows != actual.rows:
                raise DataResultDiffer("canonic and actual results have different values")
            return

        raise ProgramError("Unable to compare results", details="actual {}, canonic {}".format(actual, canonic))
